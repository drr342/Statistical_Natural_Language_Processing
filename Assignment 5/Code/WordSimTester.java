...
public class WordSimTester {
	...
	private static HashMap<String, float[]> getEmbeddings(String dataPath,
		...
		List<List<String>> sentenceCollection = new ArrayList<>(SentenceCollection.Reader
				.readSentenceCollection(dataPath + "/" + fileNames[0]));
		for (int i = 1; i < fileNames.length; i++) {
			sentenceCollection.addAll(SentenceCollection.Reader
				.readSentenceCollection(dataPath + "/" + fileNames[i]));
		}
	...
	private static HashMap<String, float[]> readEmbeddings(String path)
		...
		BufferedReader reader = new BufferedReader(new FileReader(path));
		String line = "";
		String[] firstLine = reader.readLine().trim().split("\\s{1,}");
		int vocab_size = 1, embedding_dim;
		String[] parts;
...
//				System.exit(0);
//			}
			vocab_size = Integer.parseInt(parts[0]);
			embedding_dim = Integer.parseInt(parts[1]);
		} else {
			embedding_dim = firstLine.length - 1;
