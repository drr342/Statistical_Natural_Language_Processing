...
//			}
			...
			// Traditional MaxEnt Model
			int realIndex, linearIndex;
			double count;
			// Loop all data
			for (EncodedDatum datum : data) {
				// OBJECTIVE
				logProbabilities = getLogProbabilities(datum, w, encoding, indexLinearizer);
				objective += logProbabilities[datum.getLabelIndex()];
				// DERIVATIVES
				for (int i = 0; i < datum.getNumActiveFeatures(); i++) {
					realIndex = datum.getFeatureIndex(i);
					linearIndex = indexLinearizer.getLinearIndex(realIndex, datum.getLabelIndex());
					count = datum.getFeatureCount(i);
					derivatives[linearIndex] += count;
					for (int j = 0; j < indexLinearizer.numLabels; j++) {
						linearIndex = indexLinearizer.getLinearIndex(realIndex, j);
						derivatives[linearIndex] -= Math.exp(logProbabilities[j]) * count;
					}
				}
			}
			...
			for (int i = 0; i < w.length; i++) {
				sum += w[i] * w[i];
				derivatives[i] -= w[i] / (sigma * sigma);
			}
			objective -= sum / (2 * sigma * sigma);

			// negate objective and derivatives (min -> max)
			objective = -objective;
			for (int i = 0; i < derivatives.length; i++) {
	...
	private static <F, L> double[] getLogProbabilities(EncodedDatum datum, double[] weights, Encoding<F, L> encoding,
			IndexLinearizer indexLinearizer) {
		...
		for (int i = 0; i < logProbabilities.length; i++) {
			...
			logProbabilities[i] = Math.exp(sum);
		}
		
		// normalize
		double score = Arrays.stream(logProbabilities).sum();
		for (int i = 0; i < logProbabilities.length; i++) {
			logProbabilities[i] = Math.log(logProbabilities[i] / score);
		}
