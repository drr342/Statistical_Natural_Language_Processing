...
--]]
local EncoderTreeLSTMSentiment = torch.class('treelstm.EncoderTreeLSTMSentiment')

function EncoderTreeLSTMSentiment:__init(config)
...
function EncoderTreeLSTMSentiment:linear_layer()
...
function EncoderTreeLSTMSentiment:sentiment_decoder()
...
function EncoderTreeLSTMSentiment:train(dataset, test_dataset)
...
function EncoderTreeLSTMSentiment:predict(tree, sent)
...
function EncoderTreeLSTMSentiment:predict_dataset(dataset)
...
function EncoderTreeLSTMSentiment:get_final_accuracy(dataset, type)
  local total=0
  local correct=0
  if (type == 'train') then
    dataset.size = self.max_train
...
function EncoderTreeLSTMSentiment:get_accuracy(dataset)
  local total=0
  local correct=0
...
function EncoderTreeLSTMSentiment:get_tree_accuracy(tree, inputs)
    local correct=0
    local total=0
...
function EncoderTreeLSTMSentiment:print_config()
...
function EncoderTreeLSTMSentiment:save_results(path)
...
function EncoderTreeLSTMSentiment:save(path)
  local config = {
...
function EncoderTreeLSTMSentiment.load(path)
  local state = torch.load(path)
  local model = treelstm.EncoderTreeLSTMSentiment.new(state.config)
